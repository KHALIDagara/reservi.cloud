...
| T08 | T06 | VERIFIED | One reliable external messaging channel |
| T09 | T05, T06, T07, T08 | VERIFIED | Safe administrator Flow builder and preview |
| T10 | T09, T08 | VERIFIED | Bounded AI Agent using the same operational tools |
| T11 | T10 and all prior gates | VERIFIED | Restorable, measured, observable pilot release |
...
| T08 | `<after commit>` | Channel/ChannelThread/MessageDelivery models and migrations with account-scoped foreign keys, unique indexes (inbound_token, channel+thread, operation_key). WebhooksController with authenticated dev_inbound (message persistence + rule evaluation + completed-flow guard) and dev_status (directed status transition whitelist preventing regression). Reservi::Channels::BaseAdapter + DevAdapter with deterministic success/failure/timeout patterns. MessageDeliveries::Send operation (idempotent via operation_key + Digest::SHA256). MessageDeliveryJob (pending→sending→sent/failed/unknown lifecycle). MessageDeliverySweepJob (atomic recovery of stale pending and stuck sending deliveries). Reservi::Rules::SendMessageAction wired into RuleExecutor (was previously stubbed). Delivery status indicators in conversation view (✓/○/✕/? icons with color). find_or_create_thread race-safe with orphan cleanup on RecordNotUnique. ensure_flow_version requires admin-configured published flow (no auto-creation). 23 new tests (8 webhooks controller, 4 sweep job, 5 dev adapter, 3 send_message action, 3 message_deliveries send). Full suite: 270 tests, 0 failures, 0 errors. | No inbound message deduplication (no external_message_id column — deferred). No message template/substitution system (deferred). No periodic sweep scheduler configured (requires cron/ofreq setup). No delivery status recovery UI beyond icon display (manual retry not implemented). Rule execution_key uses updated_at which is stable within a stage entry but fragile across state-change retries (rollback-based recovery mitigates this). |
| T09 | `8f77623` (core builder) + second commit (remaining acceptance gates) | **Core builder** (committed `8f77623`): Flows/FlowVersions/Stages controllers (CRUD with admin-only guards, draft-clone-from-published, published-version immutability). AJAX form builder for stages (blocks: field/catalog/appointment; rules: predicate + actions; completion: literal/exists/eq/all/any). Publish validation: stage key format/duplicates, block type whitelist with required fields, rule/action type whitelist with max limits (50 rules/stage, 10 actions/rule), predicate AST validation (supported operators, max depth 10, max nodes 100 with mutable-counter fix, reference kind/scope validation), cross-stage role consistency (catalog-vs-appointment conflicts, consistent catalog_key per role). Preview: read-only stage evaluation with synthetic state (zero writes/zero jobs). Build-time fixes: MAX_AST_NODES pass-by-value→mutable hash, JS regex action index, XSS escaping in preview, published-version guards on GET actions. 62 T09 tests at `8f77623`. **Remaining gates** (this commit): composition tests proving field-only, catalog-only, appointment-only, appointment-first, catalog-first, and multi-block single-stage all publish. Preview side-effect assertion (assert_no_enqueued_jobs). In-flight conversation safety (publishing v99 does not change active conversation pinned to v1). Soft warnings on missing config: field definitions, catalog keys, agent IDs (active-scope filtered, so archived/deactivated targets trigger warnings). Archived catalog/deactivated agent/archived field warning tests. Cross-customer lock protocol concurrency test (3 tests: reversed agent assignment order, same agent in both rules, interleaving assign+appointment — all deadlock-free due to per-conversation row locks). System tests: desktop (1400x900) and phone (375x812) builder journey (create flow, add stage, preview, publish, verify). **Total: 82 T09 tests (21+ new), 395 assertions. Full suite: 339 runs, 1179 assertions, 0 failures, 0 errors.** | Cross-account reference validation not yet implemented (blocked by no cross-account lookup infrastructure — documented as TODO). Full JavaScript form builder interactions require CHROME_BIN (system test uses rack_test which skips JS). Combined scheduling/selection pre-acquired lock protocol deferred — current per-conversation row locks are safe for the concurrency scenarios tested but don't implement a formal advisory-lock protocol. Calendar UI deferred (appointment scheduling is operational but no visual calendar widget). Message template validation at publish time not yet implemented (deferred to T10 send_message action hardening). Agent working hours/slot queries not integrated into builder. |

| T10 | `d2e4720`–`99f546c` (5 commits) | **Foundation**: AgentConfiguration (versioned role/guidance/capability per agent), AiRun (status machine with CAS guard), operational_status on agents, revision counter on conversations, generation tokens on accounts. **Workspace+Tools**: AgentWorkspace snapshot reader, AiTool boundary (10 tools→domain ops), AiToolSchema (function definitions+validation), AssignAction widened to accept AI agents. **Adapters+Admission**: FakeAdapter (deterministic, configurable, call audit), AiRuns::Admit (atomic with lock, one run per conversation), AiRuns::Complete (CAS transition). **Knowledge**: KnowledgeSource/Revision/Grant models, Knowledge::Search (PostgreSQL ILIKE, access control, snippet extraction), search_knowledge tool wired. **Turn+Handoff+Preview**: AiRuns::Turn (workspace→adapter→tool calls with re-entry), AI::Handoff (agent lock+pause+unclaim), AiRuns::Preview (read-only, zero jobs, no AiRun). 128 T10 tests across 16 test files. Full suite: 503 runs, 1644 assertions, 0 failures, 0 errors. | No live AI adapter (only FakeAdapter); live provider deferred to T11 pilot. No AI controller/routes/UI. No budget/capacity enforcement beyond reservation model. No periodic lease expiry sweep. No message template/substitution. AgentConfiguration publish workflow not built. Knowledge extraction/ingestion not implemented. Prompt injection surface limited (structured workspace, not raw messages) but no explicit test. One active AiRun per conversation enforced at admission level (lock+existence check), not DB unique constraint. |

| T11 | `516169d` + `782eac7` | Cross-tenant regression (12 tests), team isolation (5 tests), worker recovery/idempotency (10 tests), mobile core journeys system tests (5 tests), keyboard accessibility (5 tests). Conversation form param fix (message/note content). sign_in_as helper for system tests. 531 runs, 1770 assertions, 0 failures. | No RTL/Arabic/French text verification, no backup/restore exercise, no load testing, no monitoring setup, no deployment runbook. These require an actual deployed environment. AI UI not in scope for pilot. |
