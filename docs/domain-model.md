...
## 33. Account invitations, AI configuration and knowledge

See [Accounts and AI setup](accounts-and-ai-setup.md) §8 for the additional persistence contract: AccountInvitation, AgentConfiguration, KnowledgeSource, KnowledgeRevision, KnowledgeChunk and AgentKnowledgeGrant. These are lifecycle/access records, not alternative Conversation/Agent models. All owned links and current-version pointers preserve Account and parent identity.

User is global authentication; Membership binds it to an Account. Human Agent requires that membership; AI Agent must not reference a fabricated human membership. Conversation ownership and TeamMembership reference Agent for both kinds. Account creation bootstraps the creator's assignable human Agent atomically; additional people become assignable through verified invitation acceptance, including reactivation; AI becomes eligible through published configuration and activation. AI operational state is draft/active/paused/archived, distinct from its configuration drafts/versions.

Q&A and scenarios are authored KnowledgeSource kinds. Documents produce reviewed extracted revisions; chunks are rebuildable search projections. Shared published sources serve human and AI Agents without duplicate uploads; restricted grants are explicit. Behavioral instructions live in Account guidance and AgentConfiguration. No source, scenario, summary or prompt can grant capabilities or become authoritative process state.
